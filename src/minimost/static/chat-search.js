...
function populateSearchChannels() {
  ...
  document
    .querySelectorAll("#sidebar-dynamic .sidebar-item[data-channel]")
    .forEach((item) => {
      const opt = document.createElement("option");
      opt.value = item.dataset.channel;
      opt.textContent =
        item.querySelector(".label")?.textContent || item.dataset.channel;
...
const searchFromSuggestions = document.getElementById(
...
function wrapSelection(el, wrapper) {
  ...
  if (start === end) {
    ...
    el.value = value.slice(0, start) + wrapper + wrapper + value.slice(end);
...
function _handleCtrlKey(e, isInput) {
  if (!isInput) {
    ...
    if (e.key === "j") {
      ...
      switchChannel(
        items[(currentIndex - 1 + items.length) % items.length].dataset.channel,
      );
      e.preventDefault();
    }
    return;
...
searchInput.addEventListener("keydown", (e) => {
  ...
  if (e.key === "ArrowDown") {
    ...
    e.preventDefault();
    searchSelectedIndex =
      (searchSelectedIndex - 1 + items.length) % items.length;
    updateSearchHighlight();
  } else if (e.key === "Enter") {
    e.preventDefault();
...
const _HL_RULES = (() => {
  ...
  return {
    python: [
      ...
      [
        kw(
          "bool bytes bytearray complex dict float frozenset int list memoryview object set str tuple type Exception BaseException ValueError TypeError KeyError IndexError AttributeError RuntimeError StopIteration NotImplementedError OSError IOError FileNotFoundError PermissionError NameError ImportError ModuleNotFoundError ArithmeticError ZeroDivisionError OverflowError",
        ),
        "type",
      ],
      [
        kw(
          "abs all any ascii bin callable chr compile delattr dir divmod enumerate eval exec filter format getattr globals hasattr hash help hex id input isinstance issubclass iter len locals map max min next oct open ord pow print repr reversed round setattr slice sorted staticmethod classmethod property super sum vars zip",
        ),
        "bi",
      ],
      [NUM_HEX, "num"],
    ...
    js: [
      ...
      [
        kw(
          "Array Boolean Date Error Function JSON Map Math Number Object Promise Proxy Reflect RegExp Set String Symbol WeakMap WeakSet console document window globalThis parseInt parseFloat isNaN isFinite encodeURIComponent decodeURIComponent setTimeout clearTimeout setInterval clearInterval fetch",
        ),
        "bi",
      ],
      [NUM_HEX, "num"],
    ...
    c: [
      ...
      [
        kw(
          "auto break case continue default do else enum extern for goto if inline return sizeof static struct switch typedef union while namespace class template typename public private protected virtual override new delete this throw try catch operator nullptr NULL true false const_cast static_cast dynamic_cast reinterpret_cast using constexpr consteval constinit noexcept explicit friend mutable volatile register co_await co_return co_yield requires concept",
        ),
        "kw",
      ],
      [
        kw(
          "bool char double float int long short signed unsigned void wchar_t char8_t char16_t char32_t int8_t int16_t int32_t int64_t uint8_t uint16_t uint32_t uint64_t size_t ptrdiff_t ssize_t intptr_t uintptr_t auto",
        ),
        "type",
      ],
      [NUM_HEX, "num"],
    ...
    sh: [
      ...
      [
        kw(
          "if then else elif fi for while until do done case esac in function return local export readonly declare typeset select time coproc",
        ),
        "kw",
      ],
      [
        kw(
          "echo printf read cd pwd source alias unalias set unset shift trap exec eval exit test true false grep sed awk find sort cut xargs mkdir rm cp mv cat head tail",
        ),
        "bi",
      ],
      [NUM_HEX, "num"],
    ...
    make: [
      ...
      [
        kw(
          "ifeq ifneq ifdef ifndef else endif include override export unexport define endef undefine",
        ),
        "kw",
      ],
    ],
    groovy: [
      ...
      [
        kw(
          "as assert break case catch class const continue def default do else enum extends false finally for goto if implements import in instanceof interface new null package return super switch this throw throws trait true try while abstract final native private protected public static strictfp synchronized transient volatile var",
        ),
        "kw",
      ],
      [
        kw(
          "boolean byte char double float int long short void String Object List Map Set Boolean Integer Long Double Float Number BigDecimal BigInteger Closure GString",
        ),
        "type",
      ],
      [
        kw(
          "pipeline agent stages stage steps script environment options parameters triggers tools when post always success failure unstable changed cleanup parallel matrix node sh bat echo dir withEnv withCredentials checkout git input timeout retry build archiveArtifacts junit stash unstash error catchError readFile writeFile fileExists emailext docker label",
        ),
        "bi",
      ],
      [NUM_HEX, "num"],
    ...
    cmake: [
      ...
      [
        kwi(
          "if else elseif endif foreach endforeach while endwhile function endfunction macro endmacro return break continue cmake_minimum_required cmake_policy project add_executable add_library add_subdirectory target_link_libraries target_include_directories target_compile_options target_compile_definitions target_sources target_precompile_headers target_link_directories target_link_options set unset option message string list math include find_package find_library find_path find_program find_file install configure_file file execute_process add_custom_command add_custom_target add_test enable_testing enable_language include_directories link_directories link_libraries get_filename_component get_target_property set_target_properties set_property get_property add_definitions add_compile_options add_compile_definitions add_link_options cmake_parse_arguments mark_as_advanced check_cxx_compiler_flag check_c_compiler_flag check_include_file check_function_exists include_guard block endblock FetchContent_Declare FetchContent_MakeAvailable FetchContent_GetProperties FetchContent_Populate ExternalProject_Add ExternalProject_Get_Property target_compile_features write_basic_package_version_file configure_package_config_file",
        ),
        "kw",
      ],
      [
        kwi(
          "ON OFF TRUE FALSE YES NO NOTFOUND PUBLIC PRIVATE INTERFACE SHARED STATIC MODULE OBJECT ALIAS IMPORTED STATUS WARNING SEND_ERROR FATAL_ERROR DEPRECATION REQUIRED COMPONENTS OPTIONAL_COMPONENTS DESTINATION FILES PROGRAMS DIRECTORY TARGETS STREQUAL STRLESS STRGREATER EQUAL LESS GREATER MATCHES EXISTS IS_DIRECTORY IS_ABSOLUTE DEFINED COMMAND POLICY APPEND PREPEND VERSION",
        ),
        "type",
      ],
      [NUM_HEX, "num"],
    ...
    vhdl: [
      ...
      [
        kwi(
          "std_logic std_logic_vector std_ulogic std_ulogic_vector unsigned signed integer natural positive boolean bit bit_vector character string real time severity_level line text side width rising_edge falling_edge to_integer to_unsigned to_signed to_std_logic_vector to_bitvector conv_integer conv_std_logic_vector resize shift_left shift_right ieee std_logic_1164 numeric_std numeric_bit std_textio textio work",
        ),
        "type",
      ],
      [NUM_HEX, "num"],
    ...
    verilog: [
      ...
      [
        kw(
          "always always_comb always_ff always_latch assign automatic begin buf bufif0 bufif1 case casex casez deassign default defparam disable edge else end endcase endfunction endgenerate endmodule endprimitive endspecify endtable endtask event for force forever fork function generate genvar if initial inout input join localparam macromodule module nand negedge nor not or output parameter posedge primitive release repeat return specify specparam supply0 supply1 table task tri tri0 tri1 triand trior trireg wait wand while wire wor xnor xor bit byte chandle class clocking const constraint covergroup coverpoint do endclass endclocking endgroup endinterface endpackage endprogram enum extends final foreach iff import inside interface join_any join_none local modport new null package priority program property protected pure rand randc randcase ref sequence shortint shortreal static string struct super this typedef union unique unique0 var virtual void with within intersect bind checker endchecker cover expect global matches soft solve until",
        ),
        "kw",
      ],
      [
        kw(
          "logic reg integer real realtime signed unsigned int longint byte shortint time bit chandle event",
        ),
        "type",
      ],
      [VL_NUM, "num"],
    ...
    java: [
      ...
      [
        kw(
          "abstract assert break case catch class const continue default do else enum extends final finally for goto if implements import instanceof interface native new null package private protected public return static strictfp super switch synchronized this throw throws transient try volatile while true false record sealed permits yield var",
        ),
        "kw",
      ],
      [
        kw(
          "boolean byte char double float int long short void String Integer Long Double Float Boolean Byte Short Character Object Number Exception RuntimeException Error List Map Set Collection ArrayList HashMap HashSet LinkedList Optional Stream Iterator Iterable Comparable Cloneable Runnable Callable",
        ),
        "type",
      ],
      [NUM_HEX, "num"],
    ...
    go: [
      ...
      [
        kw(
          "any error bool byte rune string int int8 int16 int32 int64 uint uint8 uint16 uint32 uint64 uintptr float32 float64 complex64 complex128",
        ),
        "type",
      ],
      [
    ...
    rust: [
      ...
      [
        kw(
          "i8 i16 i32 i64 i128 isize u8 u16 u32 u64 u128 usize f32 f64 bool char str String Option Result Vec Box Arc Rc HashMap HashSet BTreeMap BTreeSet Some None Ok Err",
        ),
        "type",
      ],
      [NUM_HEX, "num"],
...
function _previewCodeEl(data) {
  ...
  if (hlStart) {
    requestAnimationFrame(() => {
      ...
      if (hlRow) {
        pre.scrollTop =
          hlRow.offsetTop - pre.clientHeight / 2 + hlRow.clientHeight / 2;
      }
    });
  }
...
(function () {
  let fontSize =
    Number.parseFloat(localStorage.getItem("chatFontSize")) ||
    CHAT_FONT_DEFAULT;
  ...
  chatEl.addEventListener(
    ...
    (e) => {
      ...
      fontSize = Math.min(
        CHAT_FONT_MAX,
        Math.max(CHAT_FONT_MIN, startSize * scale),
      );
      applyChatFontSize(fontSize);
    },
    { passive: false },
  ...
  chatEl.addEventListener(
    ...
    (e) => {
      ...
      fontSize = Math.min(
        CHAT_FONT_MAX,
        Math.max(CHAT_FONT_MIN, fontSize - e.deltaY * 0.05),
      );
      applyChatFontSize(fontSize);
      localStorage.setItem("chatFontSize", fontSize);
    },
