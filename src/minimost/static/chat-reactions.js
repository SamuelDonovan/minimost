...
const REACTION_EMOJI = Object.fromEntries(
  REACTIONS.map((r) => [r.name, r.emoji]),
);
  ...
  search.addEventListener("keydown", function (e) {
    if (e.key === "Escape") {
      ...
      const first = Array.from(
        document.querySelectorAll(".reaction-picker-item"),
...
function filterReactions(query) {
  ...
  document.querySelectorAll(".reaction-picker-item").forEach(function (item) {
    const match =
      !q || item.dataset.name.includes(q) || item.dataset.label.includes(q);
...
(function buildEmojiPicker() {
  ...
  search.addEventListener("keydown", function (e) {
    if (e.key === "Escape") {
      ...
      e.preventDefault();
      const first = Array.from(
        grid.querySelectorAll(".emoji-picker-item"),
      ).find((el) => el.style.display !== "none");
      if (first) first.click();
    }
  });
...
function filterEmojis(query) {
  const q = query.toLowerCase().trim();
  document
    .querySelectorAll("#emoji-picker-grid .emoji-picker-item")
