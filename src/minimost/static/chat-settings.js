...
document
  .getElementById("settings-avatar-file")
...
document
  .getElementById("settings-avatar-remove")
...
function _buildColorSwatches(currentColor) {
  ...
  COLOR_PRESETS.forEach((hex) => {
    ...
    sw.className =
      "color-swatch" +
      (hex.toLowerCase() === currentColor?.toLowerCase() ? " selected" : "");
    ...
    sw.addEventListener("click", () => {
      ...
      container
        ...
        .forEach((s) =>
          s.classList.toggle(
            "selected",
            s.title.toLowerCase() === hex.toLowerCase(),
          ),
        );
    });
...
function openSettings() {
  ...
  fontSlider.value = Math.round(
    Math.min(CHAT_FONT_MAX, Math.max(CHAT_FONT_MIN, fontSize)),
  );
  document.getElementById("settings-font-size-label").textContent =
    `(${fontSlider.value}px)`;

  const enterToSend = localStorage.getItem("enterToSend") !== "false";
  document.getElementById("settings-enter-key").value = enterToSend
    ? "send"
    : "newline";
  ...
  document.getElementById("native-notif-hint").style.display = notifDenied
    ? ""
    : "none";
...
document.getElementById("settings-bio").addEventListener("input", (e) => {
  document.getElementById("settings-bio-count").textContent =
...
document
  .getElementById("settings-notif-sounds")
  .addEventListener("change", _updateNotifSoundIcon);
...
document
  .getElementById("settings-native-notif")
...
document
  .getElementById("settings-name-color")
  .addEventListener("input", (e) => {
    ...
    document
      ...
      .forEach((s) =>
        s.classList.toggle(
          "selected",
          s.title.toLowerCase() === e.target.value.toLowerCase(),
        ),
      );
  });

document
  .getElementById("settings-color-reset")
  .addEventListener("click", () => {
...
document.getElementById("settings-save-btn").addEventListener("click", () => {
  ...
  // Enter key behavior
  const enterToSend =
    document.getElementById("settings-enter-key").value === "send";
...
document
  .getElementById("account-save-btn")
  .addEventListener("click", async () => {
    ...
    // Name color + bio
    const color = _settingsUseDefault
      ? null
      : document.getElementById("settings-name-color").value;
    const bio =
      document.getElementById("settings-bio").value.trim().slice(0, 160) ||
      null;
...
function showDeleteConfirm(type) {
  ...
  document.getElementById("settings-delete-confirm-btn").textContent =
    info.btnLabel;
...
async function membersForChannel(ch) {
  ...
  if (ch.startsWith("dm:")) {
    ...
    const id = ch.split(":")[1];
    const members = await fetch(`/private_channels/${id}/members`).then((r) =>
      r.json(),
    );
...
async function openUsersModal() {
  ...
  document.getElementById("users-modal-add").style.display = isPrivate
    ? "block"
    : "none";
...
async function renderMembersList() {
  ...
  try {
    ...
    list.innerHTML =
      '<div class="users-list-loading">Failed to load members.</div>';
    return;
  }
