...
def _load_valid_reactions():
    """Build the set of valid reaction emoji names from the SVG directory.

    Scans ``static/reactions/`` and returns the filenames (without the
    ``.svg`` extension) of every SVG file present.  This set is used by
    :func:`react` to validate incoming reaction names.

    If the directory cannot be read (e.g. in a stripped deployment), an
    empty set is returned and all reactions are rejected.

    This function is called once at module import time to populate the
    module-level :data:`VALID_REACTIONS` constant.

    :returns: Set of valid reaction emoji name strings.
    :rtype: set of str
    """
