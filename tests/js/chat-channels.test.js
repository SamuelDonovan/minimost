...
function makeFetchMock(overrides = {}) {
    return jest.fn().mockImplementation((url) => {
        if (url === '/channels')         return Promise.resolve({ ok: true, json: () => Promise.resolve(['general']) });
...
describe('openCreatePrivateChannel()', () => {
    ...
    test('hides suggestions', () => {
        ...
        openCreatePrivateChannel();
        expect(document.getElementById('private-ch-suggestions').style.display).toBe('none');
    });
...
describe('private-ch-create-btn', () => {
    ...
    test('creates channel on valid input', async () => {
        ...
        global.fetch = jest.fn().mockImplementation((url) => {
            if (url === '/private_channels/create') return Promise.resolve({ ok: true, json: () => Promise.resolve({ channel: 'private:2', name: 'Test Channel' }) });
            if (url === '/channels')         return Promise.resolve({ ok: true, json: () => Promise.resolve(['general']) });
            if (url === '/private_channels') return Promise.resolve({ ok: true, json: () => Promise.resolve([]) });
            if (url === '/dms')              return Promise.resolve({ ok: true, json: () => Promise.resolve([]) });
            if (url === '/user_colors')      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
...
describe('openRenameChannel()', () => {
    ...
    test('does nothing when channel is not private', () => {
        ...
        openRenameChannel();
        expect(document.getElementById('rename-private-ch-modal').style.display).toBe('none');
    });
...
describe('rename-ch-submit-btn', () => {
    ...
    test('does nothing when channel is not private', async () => {
        ...
        await Promise.resolve();
        expect(global.fetch).not.toHaveBeenCalledWith(expect.stringContaining('rename'), expect.anything());
    });
    ...
    test('shows error when name is too long', async () => {
        ...
        document.getElementById('rename-ch-submit-btn').click();
        await Promise.resolve();
        expect(document.getElementById('rename-ch-name-error').textContent).toContain('characters or fewer');
    });
    ...
    test('renames channel on valid input', async () => {
        ...
        global.fetch = jest.fn().mockImplementation((url) => {
            if (url.includes('rename')) return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
            if (url === '/private_channels') return Promise.resolve({ ok: true, json: () => Promise.resolve([]) });
            if (url === '/channel_unreads')  return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
            return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
        });
...
describe('leaveChannel()', () => {
    ...
    test('calls leave endpoint and switches to general', async () => {
        global.channel = 'private:1';
        global.fetch = jest.fn().mockImplementation((url) => {
            if (url.includes('leave')) return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
            if (url === '/private_channels') return Promise.resolve({ ok: true, json: () => Promise.resolve([]) });
            if (url === '/channel_unreads')  return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
            return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
        });
...
describe('add-member-input keyboard navigation', () => {
    ...
    test('ArrowDown moves selection down', () => {
        document.getElementById('add-member-input').dispatchEvent(makeKeyEvent('ArrowDown'));
        const items = document.getElementById('add-member-suggestions').children;
        expect(items[0].classList.contains('active')).toBe(true);
    });

    test('ArrowUp wraps to last', () => {
        document.getElementById('add-member-input').dispatchEvent(makeKeyEvent('ArrowUp'));
        const items = document.getElementById('add-member-suggestions').children;
        expect(items[1].classList.contains('active')).toBe(true);
    });

    test('Escape hides suggestions', () => {
        document.getElementById('add-member-input').dispatchEvent(makeKeyEvent('Escape'));
        expect(document.getElementById('add-member-suggestions').style.display).toBe('none');
    });
});
...
describe('private-ch-members-input keyboard navigation', () => {
    ...
    test('ArrowDown moves selection down', () => {
        document.getElementById('private-ch-members-input').dispatchEvent(makeKeyEvent('ArrowDown'));
        const items = document.getElementById('private-ch-suggestions').children;
        expect(items[0].classList.contains('active')).toBe(true);
    });

    test('ArrowUp wraps to last', () => {
        document.getElementById('private-ch-members-input').dispatchEvent(makeKeyEvent('ArrowUp'));
        const items = document.getElementById('private-ch-suggestions').children;
        expect(items[1].classList.contains('active')).toBe(true);
    });

    test('Escape hides suggestions', () => {
        document.getElementById('private-ch-members-input').dispatchEvent(makeKeyEvent('Escape'));
        expect(document.getElementById('private-ch-suggestions').style.display).toBe('none');
    });

    test('Tab selects first suggestion', () => {
        document.getElementById('private-ch-members-input').value = 'b';
        document.getElementById('private-ch-members-input').dispatchEvent(makeKeyEvent('Tab'));
        expect(document.getElementById('private-ch-suggestions').style.display).toBe('none');
    });
});
