...
describe('openReactionPicker()', () => {
    test('shows the reaction picker', () => {
        ...
        anchor.getBoundingClientRect = () => ({ left: 100, bottom: 200, right: 200 });
        anchor.currentTarget = anchor;
        document.body.appendChild(anchor);

        openReactionPicker(42, { stopPropagation: jest.fn(), currentTarget: anchor });
        expect(document.getElementById('reaction-picker').style.display).toBe('block');
        anchor.remove();
    });
    ...
    test('removes reaction-picker-open class from previous message', () => {
        ...
        openReactionPicker(99, { stopPropagation: jest.fn(), currentTarget: anchor });
        // Then open with msg 100
        openReactionPicker(100, { stopPropagation: jest.fn(), currentTarget: anchor });
...
describe('closeReactionPicker()', () => {
    test('hides the reaction picker', () => {
        ...
        closeReactionPicker();
        expect(document.getElementById('reaction-picker').style.display).toBe('none');
    });
    ...
    test('removes visual-selected class from message element', () => {
        ...
        document.body.appendChild(anchor);
        openReactionPicker(77, { stopPropagation: jest.fn(), currentTarget: anchor });
...
describe('filterReactions()', () => {
    test('hides items that do not match query', () => {
        ...
        const hidden = Array.from(items).filter(el => el.style.display === 'none');
    ...
    test('query matches label (case-insensitive)', () => {
        ...
        const items = document.querySelectorAll('.reaction-picker-item');
        const thumbsItem = Array.from(items).find(el => el.dataset.name === 'thumbsup');
...
describe('pickerReact() via openReactionPicker', () => {
    test('calls toggleReaction and closes picker', () => {
        ...
        openReactionPicker(55, { stopPropagation: jest.fn(), currentTarget: anchor });
