...
describe('openDmModal()', () => {
    test('shows the DM modal', async () => {
        global.fetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(['bob', 'charlie']) });
        await openDmModal();
...
describe('DM input keydown handlers', () => {
    ...
    test('Escape hides suggestions', () => {
        ...
        expect(document.getElementById('dm-suggestions').style.display).toBe('none');
    });
...
describe('openDmModal()', () => {
    test('shows modal', async () => {
        global.fetch.mockResolvedValue({ ok: true, json: () => Promise.resolve(['alice', 'bob']) });
        await openDmModal();
        expect(document.getElementById('dm-modal').style.display).toBe('block');
    });
    test('fetches and stores users when not loaded', async () => {
        global.usersLoaded = false;
        global.fetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(['alice', 'bob']) });
        await openDmModal();
