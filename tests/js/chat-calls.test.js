...
beforeAll(() => {
    ...
    global.fuzzySearch         = jest.fn((q, t) => {
        if (t.toLowerCase().includes(q.toLowerCase())) return { score: 1, indices: [0] };
        return null;
    });
    ...
    global.escapeHtml = (t) => t.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
    global.profileCache       = {};
    ...
    global.privateMembersModal  = document.getElementById('private-ch-members-modal');
...
beforeEach(() => {
    jest.clearAllMocks();
    global.fetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({}), text: () => Promise.resolve('') });
...
describe('updateCallButton()', () => {
    test('shows call button for DM channel', () => {
        global.channel = 'dm:alice:bob';
        updateCallButton();
        expect(document.getElementById('call-btn').style.display).toBe('inline-flex');
    });
...
describe('_diffParticipants()', () => {
    ...
    test('handles multiple participants', () => {
        expect(() => _diffParticipants(new Set(['user1', 'user2', 'user3']))).not.toThrow();
...
describe('_handleScreenshareState()', () => {
    test('changes currentScreenSender when sender changes', () => {
        ...
        expect(global.currentScreenSender || document.getElementById('call-screen-btn')).toBeTruthy();
    });
...
describe('openIncomingCallUI()', () => {
    ...
    test('sets caller name', () => {
        ...
        expect(document.getElementById('call-caller-name').textContent).toBe('charlie');
    });
});
...
describe('openActiveCallUI()', () => {
    ...
    test('clears participant grid', () => {
        document.getElementById('call-participants-grid').innerHTML = '<div>old</div>';
        openActiveCallUI();
...
describe('startCall()', () => {
    ...
    test('shows alert if not secure context', async () => {
        ...
        global.isSecureContext = true;
        global.navigator.mediaDevices = {
            getUserMedia: jest.fn().mockResolvedValue({ getAudioTracks: jest.fn(() => []), getTracks: jest.fn(() => [{ stop: jest.fn() }]) }),
            getDisplayMedia: jest.fn(),
        };
    });
    ...
    test('initiates call via fetch', async () => {
        global.fetch
            .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ call_id: 'call-test-123' }) })
            .mockResolvedValue({ ok: true, json: () => Promise.resolve({ state: 'ended', participants: [] }) });
    ...
    test('handles initiate endpoint failure', async () => {
        global.fetch.mockResolvedValueOnce({ ok: false, json: () => Promise.resolve({ error: 'busy' }) });
        await startCall();
        expect(global.alert).toHaveBeenCalled();
    });
});
...
describe('endCall()', () => {
    ...
    test('calls /calls/:id/end after starting a call', async () => {
        ...
        global.fetch = jest.fn()
            .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ call_id: 'call-end-test' }) })
...
describe('toggleAudioMute()', () => {
    ...
    test('toggles muted state after starting a call with local stream', async () => {
        ...
        global.fetch = jest.fn()
            .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ call_id: 'call-mute-test' }) })
            .mockResolvedValue({ ok: true, json: () => Promise.resolve({}) });
        await startCall();
...
describe('_renderCallInviteList()', () => {
    beforeEach(async () => {
        try { _diffParticipants(new Set()); } catch(e) {} // clear participants
...
describe('refreshScreenShares()', () => {
    ...
    test('hides banner when no other shares', async () => {
        ...
        global.fetch = jest.fn().mockResolvedValueOnce({ ok: true, json: () => Promise.resolve([]) });
        await refreshScreenShares();
        expect(document.getElementById('screenshare-banner').style.display).toBe('none');
    });
    ...
    test('shows banner when someone else is sharing', async () => {
        ...
        global.fetch = jest.fn().mockResolvedValueOnce({
            ok: true,
            json: () => Promise.resolve([{ sharer: 'bob', share_id: 'share-refresh-test' }]),
        });
        await refreshScreenShares();
        expect(document.getElementById('screenshare-banner').style.display).toBe('flex');
    });
});
...
describe('toggleStandaloneScreenShare()', () => {
    ...
    test('starts standalone share when getDisplayMedia succeeds', async () => {
        ...
        global.fetch = jest.fn().mockImplementation((url) => {
            if (url === '/screenshare/start') return Promise.resolve({ ok: true, json: () => Promise.resolve({ share_id: 'ss-1' }) });
...
describe('toggleCallInvitePanel() cached users', () => {
    test('does not fetch again when users already cached', async () => {
        ...
        global.fetch = jest.fn().mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(['bob']) });
