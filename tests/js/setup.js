...
// ── fetch mock ────────────────────────────────────────────────────────────────
const _defaultFetchResponse = () => Promise.resolve({
    ok: true,
...
global.navigator.mediaDevices  = {
    getUserMedia:    jest.fn(() => Promise.resolve({
        ...
        getVideoTracks: jest.fn(() => [{ stop: jest.fn(), addEventListener: jest.fn() }]),
...
const IDS = [
    ...
    'settings-avatar-btn', 'settings-avatar-file', 'settings-avatar-remove', 'settings-avatar-preview',
    ...
    // Sidebar
    'sidebar-dynamic', 'sidebar', 'sidebar-backdrop',
    // Calls
    'call-btn', 'topbar-share-btn', 'call-mic-level',
    'screenshare-viewer', 'screenshare-banner',
    'screenshare-banner-text', 'screenshare-banner-view-btn', 'screenshare-viewer-label',
    'call-screen-btn', 'call-invite-list', 'call-invite-search',
    'call-status', 'call-participants', 'call-actions',
    'call-panel', 'call-participants-grid', 'call-invite-panel', 'call-invite-btn',
    'call-timer', 'call-mute-audio-btn',
    'call-incoming', 'call-caller-name',
    // Chat
    ...
    'pc-member-tooltip',
    // channels rename
    'rename-ch-input', 'rename-ch-name-error',
];
...
IDS.forEach(id => {
    if (!document.getElementById(id)) {
        ...
        if (id.includes('input') || id.endsWith('-search') || id === 'dm-users' ||
            id === 'private-ch-name' || id === 'private-ch-members-input' ||
            id === 'add-member-input' || id === 'rename-ch-input' ||
        ...
        } else if (id.endsWith('-btn') || id.endsWith('-cancel') || id.endsWith('-start') ||
                   id === 'new-dm-btn' || id === 'msg-search-close') {
            const btn = document.createElement('button');
            btn.id = id;
            document.body.appendChild(btn);
        } else if (id === 'settings-notif-sounds' || id === 'settings-native-notif') {
