...
describe("formatText()", () => {
  ...
  test("inline code `text`", () => {
    expect(formatText("`code`")).toContain(
      '<code class="msg-inline-code">code</code>',
    );
  });
...
describe("enterVisualMode() / exitVisualMode()", () => {
  ...
  test("enterVisualMode selects last message and activates indicator", () => {
    ...
    expect(
      document.getElementById("msg-200").classList.contains("visual-selected"),
    ).toBe(true);
  });
  ...
  test("enterVisualMode does nothing if no messages", () => {
    ...
    expect(
      document
        .getElementById("vim-mode-indicator")
...
describe("visual mode key navigation", () => {
  ...
  test("k moves up in visual mode", () => {
    ...
    expect(
      document.getElementById("msg-20").classList.contains("visual-selected"),
    ).toBe(true);
  });
  ...
  test("ArrowUp moves up", () => {
    ...
    expect(
      document.getElementById("msg-10").classList.contains("visual-selected"),
    ).toBe(true);
  });
});
...
describe("applyChatFontSize()", () => {
  ...
  test("sets font size on typing indicator", () => {
    ...
    expect(document.getElementById("typing-indicator").style.fontSize).toBe(
      "12px",
    );
  });
});
...
describe("_handleCtrlKey()", () => {
  ...
  test("ctrl+b wraps in **", () => {
    _handleCtrlKey(makeCtrlEvent("b"), true);
    expect(wrapSelection).toHaveBeenCalledWith(expect.anything(), "**");
  });
  test("ctrl+i wraps in *", () => {
    _handleCtrlKey(makeCtrlEvent("i"), true);
    expect(wrapSelection).toHaveBeenCalledWith(expect.anything(), "*");
  });
  test("ctrl+u wraps in __", () => {
    _handleCtrlKey(makeCtrlEvent("u"), true);
    expect(wrapSelection).toHaveBeenCalledWith(expect.anything(), "__");
  });
  test("ctrl+s wraps in ~~", () => {
    _handleCtrlKey(makeCtrlEvent("s"), true);
    expect(wrapSelection).toHaveBeenCalledWith(expect.anything(), "~~");
  });
  ...
  test("ctrl+j in non-input switches to next channel", () => {
    ...
    sidebar.innerHTML =
      '<div data-channel="general">g</div><div data-channel="random">r</div>';
...
describe("_syntaxHighlight() extra languages", () => {
  test("highlights Rust keywords", () => {
    expect(_syntaxHighlight("fn main() { let x: i32 = 1; }", "rust")).toContain(
      "kw",
    );
  });
  test("highlights Go keywords", () => {
    expect(_syntaxHighlight("func main() { var x int = 1 }", "go")).toContain(
      "kw",
    );
  });
  test("highlights Java keywords", () => {
    expect(
      _syntaxHighlight("public class Foo { int x = 0; }", "java"),
    ).toContain("kw");
  });
  test("highlights C block comment", () => {
    expect(_syntaxHighlight("/* hi */ int x;", "c")).toContain("cmt");
  });
  test("highlights shell variable", () => {
    expect(_syntaxHighlight("echo $HOME", "sh")).toContain("var");
  });
  test("highlights Makefile variable", () => {
    expect(_syntaxHighlight("$(CC) -o foo", "make")).toContain("var");
  });
  test("highlights XML tag as kw", () => {
    expect(_syntaxHighlight('<root attr="val">', "xml")).toContain("kw");
  });
  test("highlights XML attribute", () => {
    expect(_syntaxHighlight('<tag attr="val">', "xml")).toContain("var");
  });
  test("escapes unknown lang", () => {
    expect(_syntaxHighlight("<b>", "")).toContain("&lt;");
  });
  test("cmake variable highlighted", () => {
    expect(_syntaxHighlight("set(${MY_VAR} 1)", "cmake")).toContain("var");
  });
});
...
describe("_previewCodeEl()", () => {
  ...
  test("shows single-line reference", () => {
    ...
    expect(el.querySelector(".preview-code-range").textContent).toContain(
      "line 5",
    );
  });
