...
beforeEach(() => {
  ...
  if (ek && ek.tagName !== "SELECT") {
    /* it's an input, that's ok */
  }
...
describe("showDeleteConfirm() / cancelDeleteConfirm()", () => {
  test('showDeleteConfirm shows delete view for "soft"', () => {
    ...
    expect(document.getElementById("account-delete-view").style.display).toBe(
      "block",
    );
    expect(document.getElementById("account-main-view").style.display).toBe(
      "none",
    );
  });
  ...
  test('showDeleteConfirm sets title for "hard"', () => {
    ...
    expect(document.getElementById("settings-delete-title").textContent).toBe(
      "Hard Delete Account",
    );
  });
...
describe("openUsersModal()", () => {
  beforeEach(() => {
    ...
    global.fetch.mockImplementation((url) => {
      ...
      if (url.startsWith("/profile/")) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ bio: "Hello" }),
        });
      }
  ...
  test("handles fetch failure", async () => {
    ...
    await openUsersModal();
    expect(document.getElementById("users-list").textContent).toContain(
      "Failed",
    );
  });
});
...
describe("openUsersModal() context awareness", () => {
    ...
    global.fetch.mockImplementation((url) => {
      ...
      if (url.endsWith("/members"))
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve([{ username: "alice" }, { username: "bob" }]),
        });
      if (url.startsWith("/profile/"))
        return Promise.resolve({ ok: true, json: () => Promise.resolve(null) });
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
  ...
  test("shows the Add Member section for private channels", async () => {
    ...
    await openUsersModal();
    expect(document.getElementById("users-modal-add").style.display).toBe(
      "block",
    );
    expect(document.getElementById("users-modal-title").textContent).toContain(
      "My Channel",
    );
  });
...
describe("settings-bio input", () => {
  test("updates character count", () => {
    ...
    expect(document.getElementById("settings-bio-count").textContent).toBe(
      "11",
    );
  });
});
...
describe("notification globals", () => {
  test("notifMuted is a boolean", () => {
    expect(typeof notifMuted).toBe("boolean");
  });
...
describe("showDeleteConfirm() / cancelDeleteConfirm()", () => {
  test("showDeleteConfirm switches to delete view", () => {
    showDeleteConfirm("soft");
    expect(
      document.getElementById("account-delete-view").style.display,
    ).not.toBe("none");
  });
  test("cancelDeleteConfirm returns to main view", () => {
    showDeleteConfirm("soft");
    cancelDeleteConfirm();
    expect(document.getElementById("account-main-view").style.display).not.toBe(
      "none",
    );
  });
});
