...
describe("defaultUserColor() mock", () => {
  test("mock returns a non-empty string", () => {
    ...
    const color =
      typeof defaultUserColor !== "undefined"
...
describe("sidebarEntry()", () => {
  ...
  test("appends to sidebar", () => {
    ...
    expect(
      document.getElementById("sidebar-dynamic").children.length,
    ).toBeGreaterThan(0);
  });
...
describe("loadSidebar()", () => {
  ...
  test("adds Public Channels section", async () => {
    ...
    const hasPublicChannels = Array.from(boldEls).some(
      (b) =>
        b.innerText === "Public Channels" ||
        b.textContent === "Public Channels",
    );
    // Also check for the channel items themselves
    const hasChannelItems =
      sb.querySelector('[data-channel="general"]') !== null;
...
describe("refreshPresence()", () => {
  test("calls /online_users", async () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ alice: "active" }),
    });
...
describe("refreshChannels()", () => {
  test("calls /channel_unreads", () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ general: 0 }),
    });
    refreshChannels();
...
describe("refreshDMs()", () => {
  ...
  test("handles case when dmHeader not found", async () => {
    ...
    global.fetch.mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve([
          { channel: "dm:alice:bob", users: ["bob"], unread: 0 },
        ]),
    });
    refreshDMs();
    await Promise.resolve();
...
// ── updateTitleBadge ──────────────────────────────────────────────────────────
describe("updateTitleBadge()", () => {
  test("shows count when unread > 0", () => {
    updateTitleBadge(5);
    expect(document.title).toContain("5");
  });
  test("clears badge when unread is 0", () => {
    updateTitleBadge(0);
    expect(document.title).not.toMatch(/^\(\d+\)/);
  });
});
